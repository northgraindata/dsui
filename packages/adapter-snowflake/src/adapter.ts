...
import { createFakeSnowflakeClient } from "./fake-client.js";
 ...
 * The fake client backs tests and local development; production wires
 * `createSnowflakeClient` from `./sql-client.js` instead.
 */
export const snowflakeAdapter = defineAdapter({
  ...
  context: (config: SnowflakeConfig): SnowflakeContext =>
    createContext(createFakeSnowflakeClient(), config),
  stores: [sessionStore, queryFiltersStore, queryEditorStore, logFiltersStore],
  resources: [
    databases,
    databaseDetails,
    schemas,
