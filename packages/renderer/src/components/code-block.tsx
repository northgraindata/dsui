...
export function CodeBlockView({ node, context }: RegistryViewProps) {
  ...
  return (
    <div className="resource-code-block">
      ...
      <div>
        <pre>
          <code data-language={language}>{highlight(value, language)}</code>
        </pre>
        <Button
          type="button"
          variant="secondary"
...
function highlight(value: string, language: string) {
  ...
  return value
    .split(
      /(\s+|'[^']*'|\b(?:SELECT|FROM|INSTALL|LOAD|CREATE|SECRET|TYPE|SET|AS|LIMIT|WHERE|WITH|INSERT|UPDATE|DELETE|DROP)\b)/gi,
    )
    .map((token, index) => {
      if (/^'[^']*'$/.test(token))
        return (
          <span className="resource-code-string" key={index}>
            {token}
          </span>
        );
      if (
        /^(SELECT|FROM|INSTALL|LOAD|CREATE|SECRET|TYPE|SET|AS|LIMIT|WHERE|WITH|INSERT|UPDATE|DELETE|DROP)$/i.test(
          token,
        )
      )
        return (
          <span className="resource-code-keyword" key={index}>
            {token}
          </span>
        );
      return token;
