...
export function PageHeaderView({
  ...
  return (
    ...
    >
      ...
      {badge?.label ? (
        <span data-tone={badge.tone ?? "info"}>{badge.label}</span>
      ) : null}
      {node.props.actions ? (
...
function nodes(
  value:
    | import("@northgraindata/dsui-adapter-sdk").PageNode
    | readonly import("@northgraindata/dsui-adapter-sdk").PageNode[],
) {
