...
const fact = Flex({
  ...
  content: [
    ...
    Stack({
      gap: "none",
      content: [Value({ field: "label" }), Value({ field: "value" })],
    }),
  ],
});
...
function Panel(path: string, title: string) {
  return Card({
    ...
    content: [
      Grid({
        columns: 2,
        content: Collection({ field: `${path}.facts`, content: fact }),
      }),
    ...
    },
    {
      ...
      content: Stack({
        content: [
          Card({
            title: "Status",
            description: { field: "tabs.0.aside.0.description" },
            content: Collection({
              field: "tabs.0.aside.0.actions",
              content: actionButton,
            }),
          }),
          Card({
...
const detailTabs = Tabs({
  items: [
    ...
    {
      label: "Configuration",
      content: Panel("tabs.1.panels.0", "Configuration"),
    },
    { label: "Examples", content: Panel("tabs.2.panels.0", "Usage examples") },
    {
      label: "Dependencies",
      content: Panel("tabs.3.panels.0", "Dependencies"),
    },
      ...
      rowActions: [
        {
          ...
          input: { name: "name" },
          when: { field: "canInstall", equals: true },
        },
        {
          label: "Load",
          action: "load-extension",
          input: { name: "name" },
          when: { field: "canLoad", equals: true },
        },
        {
          label: "Unload",
          action: "unload-extension",
          input: { name: "name" },
          when: { field: "canUnload", equals: true },
          disabledWhen: { field: "canRestart", equals: false },
        },
        {
          label: "Reload",
          action: "reload-extension",
          input: { name: "name" },
          when: { field: "canReload", equals: true },
          disabledWhen: { field: "canRestart", equals: false },
        },
      ],
    }),
      ...
      content: [
        PageHeader({
          ...
          badge: {
            ...
            tone: { field: "status.tone" },
          },
          tags: Collection({
            field: "tags",
            content: Badge({ label: { field: "value" } }),
          }),
