...
type Request = {
  ...
  id: string | number;
  method: string;
  params: { connection: Record<string, unknown>; input?: unknown };
};
...
function reply(
  ...
  process.stdout.write(
    `${JSON.stringify(message ? { jsonrpc: "2.0", id, error: { code: -32000, message } } : { jsonrpc: "2.0", id, result })}\n`,
  );
}

type BundleDefinition = {
  connectionSchema?: { parse(value: unknown): unknown };
  create?(
    context: unknown,
    connection: unknown,
  ): {
...
export async function runExternalHost(): Promise<number> {
  ...
  if (!bundle) {
    process.stderr.write(
      "external-host requires --bundle <absolute-mjs-path>\n",
    );
    return 2;
  }
  ...
  try {
    ...
    definition = (module.default ?? module.adapter) as
      | BundleDefinition
      | undefined;
    if (
      !definition?.connectionSchema?.parse ||
      typeof definition.create !== "function"
    )
      throw new Error("bundle does not export an adapter definition");
  } catch (error) {
    process.stderr.write(
      `Could not load adapter: ${error instanceof Error ? error.message : "invalid bundle"}\n`,
    );
    return 1;
  }
  ...
  for (const line of input.split(/\r?\n/)) {
    ...
    try {
      const connection = definition.connectionSchema.parse(
      ...
      const result =
        request.method === "health"
          ...
          : await instance.execute(request.method, request.params.input ?? {});
